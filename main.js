...
<div class="prek"></div><a href="#"id="back"onclick="showList()"><img src="arrow.png" alt=""> Back to shop</a>`;
let pq = [0, 0, 0];
...
function showList() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const discount = home.p[i].discountPercentage;
...
const title = home.p[0].title;
const cat = home.p.category;
const totalPrice =
  home.p[1].price * 1 + home.p[0].price * 1 + home.p[2].price * 1;
const discount = home.p.discountPercentage;
const image = home.p.thumbnail;
const brand = home.p.brand;
...
<select class="form-select" id="inputGroupSelect04" aria-label="Example select with button addon" onchange="updateTotalPrice()">
...
<input class="code2"type="text" placeholder="Enter your code                                              &rarr;" onchange="updateDiscountPrice()">
<div class="line"></div><div class="sp"><p class"ice">TOTALPRICE</p><p id="total1">€${totalPrice}.00</p></div>
...
function updateDiscountPrice() {
  ...
  //if ((document.querySelector(".code2").innerText = "FULLSTACK")) {
    const totalWithDsc = Math.round((totalPrice / 100) * 90);
